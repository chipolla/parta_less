...
?>
<?php $backgroundimage;?>
<div id="mod-tlp-parentalcontrol-main" <?php if($backgroundimage!=''){?> style=" background: url(<?php echo $backgroundimage;?>) no-repeat center center;" <?php }?>>
    <div id="mod-tlp-parentalcontrol" class="owl-carousel owl-theme">
			 <?php $i=0;
                    ...
                    ?>
                          <div class="item">
							<p class="img-ara">
                             <?php
								if (!empty($row->profile_image)){
									if($readmore==1){
								?>
                                <a href="<?php echo JRoute::_('index.php?option=com_parentalcontrol&view=parentalcontrol&id='.(int) $row->id.'&Itemid='.$parentalcontrolmenuid); ?>">
							...
							<?php }?>
                               ...
                               <?php if($readmore==1){?>
									<?php echo substr($row->parentalcontrol,0,$character_limit).' ...';?>
                                <?php }else{ echo $row->parentalcontrol;}?>
                                </p>
								<?php if($readmore==1){?>
                                <div class="read-more">
                                 <a href="<?php echo JRoute::_('index.php?option=com_parentalcontrol&view=parentalcontrol&id='.(int) $row->id.'&Itemid='.$parentalcontrolmenuid); ?>"> <?php echo $readmore_text; ?></a>
                                 </div>
                                <?php }?>
                          </div>
     		<?php } ?>
...
jQuery(document).ready(function() {
  jQuery("#mod-tlp-parentalcontrol").owlCarousel({
